...
投稿内の画像が Obsidian のローカル保管庫からのものである場合、Quail の Obsidian プラグインはそれらを読み取り、投稿を保存または公開するときに自動的に Quail にアップロードできます。
Obsidian の内部リンク構文と一般的な Markdown 画像構文の両方を使用して、投稿に画像を挿入できます。

:::tip
画像が Quail にアップロードされると、Quail は自動的に画像を WebP 形式に変換し、投稿内の画像リンクをアップロードされた画像の URL に置き換えます。
:::

**内部リンク構文**

文内の絶対パスまたは画像のエイリアシングのいずれかになります。
...
上記のコードは、画像 `image-1.png` と `image-2.png` を投稿に挿入します。 この場合、投稿内でこの名前を持つファイルは `image-1.png` だけであり、`image-2.png` は `images` フォルダー内のファイルです。
クリップボードから画像を直接貼り付けることもでき、Quail がそれを自動的に処理します。

**マークダウン画像の構文**

この方法で画像を挿入するには絶対パスを使用してください。以下に例を示します。
